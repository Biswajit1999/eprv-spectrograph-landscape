...
async function main(){
  ...
  document.querySelector('#current-focus-card').innerHTML=`<header><span class="eyebrow">Current reading focus</span><h2>EXOhSPEC: earlier work and the present system</h2><p>Many hours of design, integration, testing and analysis have gone into EXOhSPEC's individual systems, including environmental sensing, thermal control, active optics, fibre behaviour, calibration and laboratory verification.</p><p>The sources collected here describe earlier EXOhSPEC designs, prototypes and subsystem experiments. They do not publish or validate the present instrument configuration. The current system and its results are intended for a future publication once the analysis and supporting evidence are ready.</p></header><div class="dossier-index"><article><span>${esc(exohspec.paper_count)} selected sources</span><h3>${esc(exohspec.title)}</h3><p>${esc(exohspec.short_summary)}</p><p class="reading-boundary"><b>Status boundary:</b> Design goals, laboratory measurements, deployment history and current status are kept separate. No selected source is used to claim completed stellar-RV performance for the Thai instrument. ${esc(exohspec.current_status)}</p><button type="button" class="button primary" onclick="openInstrumentProfile('tno-exohspec')">Read the EXOhSPEC note</button></article></div>`;
